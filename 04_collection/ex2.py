...
print(result)
# 길이가 5 이상인 이름만 뽑기
result = [item for item in names if len(item) >= 5]
print(result)
# 중첩 for문도 가능
# x = 0, 1, 2
# y = 0, 1, 2
...
result = sum(
    [x for x in range(101) if x % 3 == 0 or x % 5 == 0]
...
result = [item for item in words if "n" in item]
...
std3 = scores[2]
result = [
    sum(score) / len(score) for score in zip(std1, std2, std3)
]  # == result = [sum(score) for score in zip(*scores)]
